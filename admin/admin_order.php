...
$result = mysqli_query($conn,"SELECT * FROM daftar_order ORDER BY kode_order DESC LIMIT $recno,$rowperpage");
...
$hasil = mysqli_query($conn,"SELECT * FROM pembeli WHERE kode_order= '".$row['kode_order']."'");
...
if ($data = mysqli_fetch_array($hasil,MYSQLI_ASSOC))
       {
         ...
         echo"<a href=\"".$_SERVER['PHP_SELF']."?id=".$row['kode_order']."&amp;act=update\"> <input type='button' value='Lunas' class='btn_submit'></a> "
            ...
            $orders_info=@mysqli_result(@mysqli_query($conn,"UPDATE daftar_order SET status ='".$status."' WHERE kode_order='".$_GET['id']."'"));
         }
                 $no++;
