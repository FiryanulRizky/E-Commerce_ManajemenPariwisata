...
$cek = mysqli_query($conn,"SELECT * FROM testimoni WHERE komentar ='".$_POST['komentar']."'");
...
if (empty($_POST['nama'])) $err['nama']="<span class=\"err\">Silahkan Isi Nama Anda</span>\n";
 ...
 if (!preg_match("/^[a-z0-9\_\.\-]+\@[a-z0-9\_\.\-]+$/i",$_POST['email'])) $err['email']="<span class=\"err\">Email &quot;".$_POST['email']."&quot; Anda Tidak valid.</span>\n";
   ...
   $hasil = mysqli_query($conn,"INSERT INTO testimoni (nama, email, komentar, commentDate)
           VALUES ('$nama', '$email', '$komentar', '$tglKomentar')");

if( !empty($_FILES['gambar']['name']) )
    {
    $path = "gambar2/";
    $lastid=@mysqli_result(@mysqli_query($conn,"SELECT id_testimoni FROM testimoni ORDER BY id_testimoni DESC LIMIT 0,1"),0,0);
...
// *** QUERY SEARCH
//echo "[ $qry_0.$qry_t ]";
$total_rec=@mysqli_num_rows(mysqli_query($conn,"SELECT id_testimoni FROM testimoni")); // *** TOTAL RECORD PRODUCT
...
$hasil = mysqli_query($conn,"SELECT * FROM testimoni ORDER BY id_testimoni DESC LIMIT $recno,$rowperpage");
